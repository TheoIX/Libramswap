...
LibramSwapFrame:SetScript("OnEvent", function()
    ...
    elseif event == "BAG_UPDATE" then
        ...
        BuildBagIndex()
    elseif event == "ADDON_LOADED" and arg1 == "LibramSwap" then
        if LibramSwapEnabled == nil then LibramSwapEnabled = true end
        ...
        LibramSwapFrame:UnregisterEvent("ADDON_LOADED")
    end
end)
...
local Original_Cast = Cast
function Cast(spellName, bookType)
    if LibramSwapEnabled then
        local libram = ResolveLibramForSpell(spellName)
        if libram then
            local ready = IsSpellReady(spellName)
    ...
    return Original_Cast(spellName, bookType)
end
